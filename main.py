...
try:
    ...
    if not settings.env["api_key"]:
        raise Exception(f"Invalid or non-existent Youtube API key")
    if not settings.args.channel and not settings.args.playlist and not settings.args.get_playlist:
